...
(function(root){
 ...
 const api={preferences,invoke,info:null,ready:null,
  ...
  mount(app){
   ...
   const showUpdate=version=>{let notice=document.getElementById('nativeUpdateNotice');if(!version){notice?.remove();return;}if(!notice){notice=document.createElement('aside');notice.id='nativeUpdateNotice';notice.style.cssText='position:fixed;right:18px;top:72px;z-index:2147482000;display:flex;align-items:center;gap:10px;max-width:440px;padding:12px 14px;border:1px solid #78bdca;border-radius:9px;background:#eefafb;color:#17465a;box-shadow:0 8px 28px #15364a2b;font:12px Segoe UI';const copy=document.createElement('span'),close=document.createElement('button');copy.dataset.copy='';close.className='btn';close.textContent='Tamam';close.onclick=()=>notice.remove();notice.append(copy,close);document.body.append(notice);}notice.querySelector('[data-copy]').textContent='ASMach '+version+' sürümü mevcut. Kurulum paketini yöneticinizden alın.';};
   const checkUpdate=async(manual=false)=>{try{if(manual)$('[data-native-status]').textContent='Denetleniyor…';const version=await invoke('check_update');$('[data-native-status]').textContent=version?'Yeni sürüm mevcut: '+version+' · Kurulum paketini yöneticinizden alın.':'Uygulama güncel.';showUpdate(version);return version;}catch(e){$('[data-native-status]').textContent=String(e);if(manual)reportError(e);return null;}};
