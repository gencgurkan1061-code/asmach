...
(function(root){
 ...
 const extraSpecs=[['chamferAngle','Pah açısı (°)',['Pah'],true],['chamferAngleTolerance','Açı ± (°)',['Pah'],true],['threadPitch','Diş adımı',['Diş'],true],['threadClass','Diş sınıfı',['Diş'],false],['fitClass','Geçme sınıfı',['Geçme'],false],['specialDesignator','Yüzey parametresi',['Yüzey'],false]];
 function compactFields(form){
  ...
  for(const [key,label,types,numeric] of extraSpecs){const field=el('label','field',label),input=el('input');input.id='cwExtra_'+key;input.type='text';input.setAttribute('aria-label',label);if(numeric)input.inputMode='decimal';field.append(input);section.append(field);
   input.addEventListener('change',()=>{const r=app.selectedAnnotation();if(!r||!types.includes(r.type))return;const value=input.value.trim(),normalized=value.replace(',','.');
    ...
    app.checkpoint();let next={...r,[key]:numeric?normalized:value,manualFields:[...new Set([...(r.manualFields||[]),key])]};next=root.ASMachRequirements.syncRequirement(next);Object.assign(r,next);app.recordChange('Türe özel karakteristik alanı güncellendi',r);app.renderAll();
 ...
 function updateFields(record){
  ...
  if(!angleMode){
    ...
    angleMode.onchange=()=>{const r=app.selectedAnnotation();if(!r)return;app.checkpoint();r.angleFormat=angleMode.value;r.unit='°';app.recordChange('Açı giriş biçimi güncellendi',r);app.renderAll();};
  }
  ...
  for(const [key,,types] of extraSpecs){const input=$('cwExtra_'+key);input.closest('.field').hidden=!types.includes(type);input.value=record[key]??'';input.setCustomValidity('');}
 ...
 function syncDraftGuard(){
  ...
  if(inspectorTabs&&!$('cwCharacteristicInfo')){
   ...
   const oldReplace=$('ipReplaceTolerance').closest('label');oldReplace.style.setProperty('display','none','important');const policy=el('label','field','Uygulama'),select=el('select');select.id='ipTolerancePolicy';select.setAttribute('aria-label','Genel tolerans uygulama şekli');select.append(new Option('Yalnızca tolerans yoksa uygula','missing'),new Option('Mevcut toleransın yerine uygula','replace'));policy.append(select);$('ipApplyGeneral').before(policy);const scope=el('small','','Kapsam: seçili karakteristik');$('ipApplyGeneral').before(scope);
  ...
  if($('ipTolerancePolicy')&&$('cwCharacteristicInfo').dataset.record!==app.selectedAnnotation()?.id){$('cwCharacteristicInfo').dataset.record=app.selectedAnnotation()?.id||'';$('ipTolerancePolicy').value='missing';}
