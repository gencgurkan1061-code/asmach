(function(root){
 ...
 function groupRows(app){
  ...
  for(const r of app.state.annotations){const row=rows.get(r.id),dot=row?.querySelector('.balloon-dot');if(!dot)continue;dot.classList.add('repeat-balloon-preview');const ns='http://www.w3.org/2000/svg',svg=document.createElementNS(ns,'svg');svg.setAttribute('viewBox','0 0 48 32');svg.setAttribute('width','48');svg.setAttribute('height','30');svg.setAttribute('aria-label','Karakteristik '+r.number);const shape=document.createElementNS(ns,r.shape==='square'?'rect':r.shape==='hexagon'?'polygon':'ellipse');if(r.shape==='square'){for(const [k,v] of Object.entries({x:7,y:3,width:34,height:26,rx:2}))shape.setAttribute(k,v);}else if(r.shape==='hexagon')shape.setAttribute('points','3,16 12,3 36,3 45,16 36,29 12,29');else for(const [k,v] of Object.entries({cx:24,cy:16,rx:18,ry:13}))shape.setAttribute(k,v);shape.setAttribute('fill','white');shape.setAttribute('stroke','currentColor');shape.setAttribute('stroke-width','1.8');const text=document.createElementNS(ns,'text');text.setAttribute('x','24');text.setAttribute('y','17');text.setAttribute('text-anchor','middle');text.setAttribute('dominant-baseline','middle');text.setAttribute('font-size',String(Math.min(13,34/Math.max(2,String(r.number).length)*1.5)));text.setAttribute('fill','currentColor');text.textContent=r.number;svg.append(shape,text);dot.replaceChildren(svg);}
...
let anchor=parent;for(const child of children){const row=rows.get(child.id);anchor.after(row);anchor=row;row.hidden=parent.hidden||collapsed.has(id);row.dataset.repeatParent=id;const number=row.querySelector('[data-editable=number]');if(number){number.style.paddingLeft='24px';const dot=number.querySelector('.balloon-dot');if(dot){dot.style.color=child.color||app.state.annotations.find(r=>r.id===id)?.color||'#07899a';}number.title='Miktara bağlı karakteristik.';}}
  }
 }
