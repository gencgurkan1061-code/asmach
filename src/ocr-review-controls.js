...
(function(root){
  ...
  const extraFields=[['threadPitch','DİŞ ADIMI'],['threadClass','DİŞ SINIFI'],['chamferAngle','PAH AÇISI (°)'],['chamferAngleTolerance','AÇI TOLERANSI (±°)'],['specialDesignator','YÜZEY PARAMETRESİ'],['datumRefs','DATUM TANIMI']];
  let config=null;
  let roleEditor=null;
  function profile(type){
    ...
    return {...p,lower:p.numeric&&!gdt&&type!=='Diş',upper:p.numeric&&type!=='Diş',general:['Ölçü','Uzunluk','Çap','Yarıçap','Açı','Pah'].includes(type),
      extras:type==='Diş'?['threadPitch','threadClass']:type==='Pah'?['chamferAngle','chamferAngleTolerance']:type==='Yüzey'?['specialDesignator']:type==='Datum'?['datumRefs']:[]};
  }
  ...
  function start(options){
    ...
    extraFields.forEach(([key,label])=>{const field=document.createElement('label');field.className='field';field.dataset.extraField=key;field.textContent=label;field.hidden=true;
      ...
      input.oninput=()=>{const pending=options.getPending();if(!pending)return;pending.structuredEdits={...pending.structuredEdits,[key]:input.value.trim()};options.onChange();};
    });
  }
