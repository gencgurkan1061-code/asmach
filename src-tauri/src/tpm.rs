...
pub struct Identity {pub device_id:String,pub binding:String,pub public_key:String,pub challenge:String,pub signature:String}
pub fn identity(create:bool)->Result<Identity>{identity_challenge(create,None)}
pub fn identity_challenge(create:bool,provided:Option<[u8;32]>)->Result<Identity>{identity_for(create,provided,false)}
pub fn online_identity(create:bool,provided:Option<[u8;32]>)->Result<Identity>{identity_for(create,provided,true)}
fn identity_for(create:bool,provided:Option<[u8;32]>,software:bool)->Result<Identity>{
 #[cfg(windows)] {
  ...
  let script=format!("& {{ {} }} -Create:${} -Software:${}",include_str!("../../scripts/tpm-license-identity.ps1"),if create{"true"}else{"false"},if software{"true"}else{"false"});
  ...
  if !output.status.success(){return Err(if software{"Çevrimiçi lisans anahtarı oluşturulamadı. Aynı Windows hesabıyla yeniden deneyin."}else{"TPM lisans anahtarı kullanılamadı. TPM kimliğini oluşturun; TPM hazır ve aynı Windows hesabı açık olmalı. TPM'yi sıfırlamayın."}.into())}
  ...
  if public.len()!=72||&public[..4]!=b"ECS1"||public[4..8]!=[32,0,0,0]||identity.binding!=if software{"software-cng-v1"}else{"tpm-cng-v1"}||STANDARD.decode(&identity.challenge).map_err(err)?!=challenge{return Err("Lisans anahtar kanıtı geçersiz.".into())}
  ...
  #[cfg(not(windows))] {let _=(create,provided,software);Err("Windows lisans sağlayıcısı gerekli.".into())}
}
